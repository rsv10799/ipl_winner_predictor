...
target = int(st.number_input('Target', step=1, min_value=1))
...
with col3:
    score = int(st.number_input('Score', step=1, min_value=0))

# Creating a numeric input for the number of overs completed
with col4:
    overs = int(st.number_input('Overs Completed', step=1, min_value=0, max_value=20))

# Creating a numeric input for the number of wickets fallen
with col5:
    wickets = int(st.number_input('Wickets Fallen', step=1, min_value=0, max_value=10))

# Checking for different match results based on the input provided
if score > target:
    st.write(f'{battingteam} won the match')

elif score == target - 1 and overs == 20:
    st.write("Match Drawn")

elif wickets == 10 and score < target - 1:
    st.write(f'{bowlingteam} won the match')

elif wickets == 10 and score == target - 1:
    st.write('Match tied')

elif battingteam == bowlingteam:
    st.write('Please select different teams. No match can be played between the same teams.')

else:
    # Checking if the input values are valid or not
    if 0 <= target <= 300 and 0 <= overs <= 20 and 0 <= wickets <= 10 and 0 <= score <= target:
        try:
